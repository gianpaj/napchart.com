...
/*
     FILE ARCHIVED ON 15:58:41 Aug 4, 2015 AND RETRIEVED FROM THE
...
ctx.beginPath();
ctx.arc(senter,senter, sirkel4,klokkegrader(data['alfa'][i].start),klokkegrader(data['alfa'][i].end), false);
f=minutesToXY_OIC(data.alfa[i].end,73);
ctx.lineTo(f.x,f.y);
ctx.arc(senter, senter, sirkel3,klokkegrader(data.alfa[i].end),klokkegrader(data.alfa[i].start), true);
ctx.save();
